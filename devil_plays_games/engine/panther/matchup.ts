...
import {
  ...
  playTricks, Story, firstLeadSeat,
} from "./panther.js";
...
async function runMatchup(
  ...
  for (let d = 0; d < N; d++) {
    ...
    // Set this hand's trump per policy (oracle/random/fixed).
    const choice = await chooseTrump(
      trumpPolicy, cfg, PLAYERS, DEALER, PANTHER, dealSeed, allowPerilsOnly, trumpIters, dealSeed * 7 + 1);
    // Use Fight as the dummy plan — matchup.ts analyses trick counts, not points.
    const story: Story = { plan: "Fight", ground: choice.perilsOnly ? null : choice.trump };
    st.vars.trump = story.ground;
    st.emit("CurseResult", { panther: PANTHER, plan: story.plan, ground: story.ground });
